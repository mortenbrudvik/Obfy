...
With `packing.enabled` (config-only; off in every preset), Obfy replaces the obfuscated PE with a framework-dependent native Windows host. User IL is AES-256 ciphertext in an overlay. Set `packing.rid` to `portable` for the previous managed `{name}.launcher.exe`. Not Pre-JIT, not self-contained, not ARM64. Packing hides the managed PE on disk; the key is in the overlay — this is obfuscation, not confidentiality.
